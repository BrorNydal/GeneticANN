...
namespace ANN
{
    ...
    public class NeuralNetwork : MonoBehaviour
    {
        ...
        public float[] LastOutput
        {
            get
            {
                ...
                for (int i = 0; i < output.Length; i++)
                    output[i] = neurons[GetNeuronIndex(NumberOfLayers - 1, i)];
                return output;
            }
        }
        ...
        {
            int w = UnityEngine.Random.Range(0, weights.Length);
            weights[w] = UnityEngine.Random.Range(-1f, 1f);
        }

        public void TweakRandomWeight(float tweak)
        {
            int w = UnityEngine.Random.Range(0, weights.Length);
            weights[w] += UnityEngine.Random.Range(-tweak, tweak);
            weights[w] = Mathf.Clamp(weights[w], -1f, 1f);
        }

        public void MutateRandomBias()
        {
            int b = UnityEngine.Random.Range(0, biases.Length);
            biases[b] = UnityEngine.Random.Range(-1f, 1f);
        }

        public void TweakRandomBias(float tweak)
        {
            int b = UnityEngine.Random.Range(0, biases.Length);
            biases[b] += UnityEngine.Random.Range(-tweak, tweak);
        ...
        public void ExtractNeuralNetwork(string name)
        {
            ...
            try
            {
                ...
                NeuralNetworkData data = CreateNeuralNetworkData();
